...
class ClientPortalEnhanced(CustomerPortal):
    ...
    def portal_get_guard_locations(self, site_id=None, **kwargs):
        ...
        # Build domain for site-based filtering
        site_domain = [('site_id', 'in', user.site_ids.ids)]
        if site_id:
            site_domain = [('site_id', '=', int(site_id))]
    ...
    def portal_feedback_submit(self, **post):
        ...
        if resident:
            ...
            vals['client_id'] = request.env['client.site'].browse(int(post.get('site_id'))).client_id.id
        
        if post.get('shift_id'):
            vals['shift_id'] = int(post.get('shift_id'))
        
        try:
            feedback = request.env['client.feedback'].create(vals)
