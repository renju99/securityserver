 ...
 */
export const generateRouteTrackingPDF = (routeData: any, addresses: Record<string, string> = {}) => {
    ...
    try {
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                0: { fontStyle: 'bold', cellWidth: 55, textColor: [107, 114, 128] as any },
                1: { textColor: [31, 41, 55] as any }
            },
            margin: { left: 14 },
            headStyles: { fillColor: [102, 126, 234] as any }
        });
        ...
        if (stops.length > 0) {
            ...
            autoTable(doc, {
                ...
                headStyles: { fillColor: [107, 114, 128] as any },
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                0: { cellWidth: 10 },
                1: { cellWidth: 40 },
                3: { cellWidth: 35, fontStyle: 'italic', textColor: [102, 126, 234] as any }
            },
            didDrawCell: (data) => {
 ...
 */
export const generateIdleReportPDF = (idleSpots: any[], staffId: string, staffName: string, startDate: string, endDate: string, threshold: number, addresses: Record<string, string> = {}) => {
    ...
    try {
        ...
        autoTable(doc, {
            ...
            styles: { fontSize: 10, cellPadding: 3 },
            columnStyles: {
                0: { fontStyle: 'bold', cellWidth: 40, textColor: [107, 114, 128] as any },
                1: { textColor: [31, 41, 55] as any }
            },
            margin: { left: 14 }
        });
        ...
        autoTable(doc, {
            ...
            margin: { left: 14 },
            headStyles: { fillColor: [118, 75, 162] as any }
        });
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                ...
                2: { cellWidth: 20 },
                3: { cellWidth: 20 },
                5: { cellWidth: 35, fontStyle: 'italic', textColor: [118, 75, 162] as any }
            },
            didDrawCell: (data) => {
                // Make GPS coords clickable
 ...
 */
function getIntervalBreakdown(locations: any[], intervalMins = 60) {
 ...
 */
function detectStops(locations: any[], stopThresholdMins = 5) {
    ...
    for (let i = 1; i < locations.length; i++) {
        const prev = locations[i - 1];
        const curr = locations[i];
        const dist = haversine(parseFloat(prev.latitude), parseFloat(prev.longitude), parseFloat(curr.latitude), parseFloat(curr.longitude));

        if (dist < distThreshold) {
            if (!currentStop) {
                currentStop = {
                    startTime: prev.timestamp,
                    lat: parseFloat(prev.latitude),
                    lng: parseFloat(prev.longitude),
                    points: [prev, curr]
                };
            } else {
            ...
            if (currentStop) {
                ...
                const duration = Math.round((new Date(endTime).getTime() - new Date(currentStop.startTime).getTime()) / 60000);
