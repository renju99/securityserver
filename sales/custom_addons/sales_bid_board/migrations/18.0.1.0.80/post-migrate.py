def migrate(cr, version):
    """Refresh proposal scope_total from enquiries; clear web asset caches (idempotent)."""
    from odoo import SUPERUSER_ID, api

    env = api.Environment(cr, SUPERUSER_ID, {})
    proposals = env["bid.proposal"].sudo().search([])
    if proposals:
        proposals._compute_proposal_scope_total()
